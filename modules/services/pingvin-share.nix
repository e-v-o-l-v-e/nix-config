{
  flake.modules.nixos.pingvin-share =
    { config, lib, ... }:
    let
      cfg = config.server;
      fqdn = cfg.domain;
    ...
    in
    {
      ...
      options.server.docker.pingvin-share-x.enable =
        lib.mkEnableOption "pingvin-share-x docker container";
    };
}
